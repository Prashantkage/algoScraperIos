    ...
    // 1. DYNAMIC SYSTEM PATH ENFORCEMENT
    // Ensures system CLI tools (lsof, kill, xcrun, node) resolve dynamically across all macOS machines
    const extraPaths = [
        '/usr/local/bin',
        '/opt/homebrew/bin',
        '/usr/bin',
    ...
    // 2. HELPER TO KILL ANY STUCK APPIUM PROCESS ON PORT 4723
    function killExistingAppium() {
        return new Promise((resolve) => {
            console.log("Cleaning up any existing process on port 4723...");
            const command = 'lsof -ti:4723 | xargs kill -9';

            exec(command, (error, stdout, stderr) => {
                if (error) {
                    console.log("Port 4723 was clean or command exited cleanly.");
                } else {
                    console.log("Successfully killed existing process on port 4723");
                }
                setTimeout(resolve, 800); // Small delay to allow OS to release the port
            });
        });
    }
    ...
    *{
    ...
    app.on('ready', async () => {
        ...
        await showStep("Checking Prerequisites", 500);
        ...
        await showStep("Launching AlgoScraper", 500);
